...
int perturbations_einstein(
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      /* first equation involving total density fluctuation */ //GDM modification
      ppw->pvecmetric[ppw->index_mt_h_prime] =
        ( k2 * s2_squared * y[ppw->pv->index_pt_eta] + 1.5 * a2 * (ppw->delta_rho - pba->gdm_epsilon * ppw->pvecback[pba->index_bg_rho_cdm] * ppw->pv->y[ppw->pv->index_pt_delta_cdm] ) )/(0.5*a_prime_over_a);  /* h' */
      ...
      ppw->pvecmetric[ppw->index_mt_h_prime_prime] =
        ...
        - 9. * a2 * (ppw->delta_p + pba->gdm_epsilon * ppw->pvecback[pba->index_bg_rho_cdm] * ppw->pv->y[ppw->pv->index_pt_delta_cdm]); //GDM modification
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      if (ppt->gauge == synchronous) {
        ...
        /*GDM add begin*/
          dy[pv->index_pt_delta_cdm] = - (y[pv->index_pt_theta_cdm] + metric_continuity)/(1.0 - pba->gdm_epsilon);
          dy[pv->index_pt_theta_cdm] = - a_prime_over_a*y[pv->index_pt_theta_cdm]*(1.0 - 4.0*pba->gdm_epsilon)/(1.0 - pba->gdm_epsilon) + pba->gdm_epsilon*k*k*dy[pv->index_pt_delta_cdm];
        /*GDM add end*/
      }
    }
